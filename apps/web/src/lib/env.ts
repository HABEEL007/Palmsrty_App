...
const envSchema = z.object({
  ...
  VITE_SUPABASE_ANON_KEY: z.string().min(1).refine(
    (key) => !key.startsWith('sb_secret') && !key.includes('service_role'),
    { message: "DO NOT use the Service Role key in the frontend. Use the 'anon' (public) key." }
  ),
