...
export const ProcessingView: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const progressTimer = setInterval(() => {
      setProgress(p => { if (p >= 100) { clearInterval(progressTimer); return 100; } return p + 1.2; });
    }, 85);

    // Call AI API and navigate to results
    const runAnalysis = async () => {
      try {
        // TODO: Replace with real API call
        // const result = await readingApi.analyze(imageData);
        // setReadingResult(result);

        // Demo: simulate API delay then navigate
        await new Promise(resolve => setTimeout(resolve, 8000));
        setReadingResult(DEMO_RESULT);
  ...
  }, [navigate, imageData, setReadingResult]);
